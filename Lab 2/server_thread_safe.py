#!/usr/bin/env python3
"""
HTTP Server with Request Counter - THREAD-SAFE VERSION
This version uses a lock to prevent race conditions.
"""
import os
import sys
import socket
...
# Global dictionary to track file hit counts
file_hits = {}

# Global lock to make counter thread-safe
...
def generate_directory_listing(dir_path, url_path):
    """Generate HTML page for directory listing with hit counts."""
    html = f"""<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <p class="success">✅ THREAD-SAFE VERSION - No race conditions!</p>
    <table>
        <tr>
...
"""
    ...
    try:
        ...
        for item in items:
            ...
            # Get hit count for this item (thread-safe read)
            with counter_lock:
...
def handle_client(client_socket, client_address, base_dir):
    ...
    try:
        ...
        # ============================================
        # INCREMENT COUNTER - THREAD-SAFE with LOCK!
        # ============================================
        with counter_lock:  # Lock ensures only one thread can access at a time
            # Read current count
            current_count = file_hits.get(file_path, 0)

            # Add artificial delay (same as naive version)
            # But now it doesn't matter - we have the lock!
            time.sleep(0.001)  # 1 millisecond delay

            # Write new count (thread-safe!)
            file_hits[file_path] = current_count + 1

            print(f"[{thread_id}] Counter for {file_path}: {file_hits[file_path]}")
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python server_thread_safe.py <directory>")
        sys.exit(1)
    ...
    print(f"✅ THREAD-SAFE HTTP Server running on http://{host}:{port}")
    print(f"Serving files from: {os.path.abspath(base_dir)}")
    print("This version uses locks to prevent race conditions!")
