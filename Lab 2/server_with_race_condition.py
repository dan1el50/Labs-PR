#!/usr/bin/env python3
"""
HTTP Server with Request Counter - NAIVE VERSION (Race Condition)
This version has a race condition bug on purpose to demonstrate the problem.
"""
import os
import sys
import socket
...
# Global dictionary to track file hit counts (NOT thread-safe!)
file_hits = {}
...
<body>
    ...
    <p class="warning">⚠️ NAIVE VERSION - Race condition present!</p>
    <table>
        <tr>
...
def handle_client(client_socket, client_address, base_dir):
    ...
    try:
        ...
        # Add artificial delay to force thread interleaving
        # This makes the race condition much more likely to occur
        time.sleep(0.001)  # 1 millisecond delay

        # Write new count (without lock - NOT thread-safe!)
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python server_with_race_condition.py <directory>")
        sys.exit(1)
    ...
    print(f"⚠️  HTTP Server with RACE CONDITION running on http://{host}:{port}")
    print(f"Serving files from: {os.path.abspath(base_dir)}")
    print("This version demonstrates race condition bug!")
